...
// Tabela de decodificação para o display de 7 segmentos (Catodo Comum)
...
void inicializa() {
    ...
    *p_ddrc = *p_ddrc & ~(0x08); // Zera o bit 3 do DDRC (configura como entrada)
...
void escreve_displays(int centena, int dezena) {
    ...
    // O display das dezenas (direita) está nos pinos digitais 0 a 6 (PORTD 0 a 6).
    // O segmento A do display das centenas (esquerda) está no pino 7 (PORTD 7).
    *p_portd = (seg_dezena & 0x7F) | ((seg_centena & 0x01) << 7);

    // Os segmentos B a G do display das centenas ocupam os pinos digitais 8 a 13 (PORTB 0 a 5).
    // Deslocamos 'seg_centena' 1 bit para alinhar com os pinos de saída correspondentes.
    *p_portb = (*p_portb & 0xC0) | ((seg_centena >> 1) & 0x3F);
}
...
int main(void) {
    ...
    while (1) {
        ...
        frequencia_rpm = (contador_pulsos * 60) / 49;
